...
select e.ename as "이름", e.job as "업무", d.dname as "부서이름" from emp e, dept d where e.deptno = d.deptno
And d.loc = 'DALLAS';
...
--CLARK	   ACCOUNTING
--JAMES	   SALES        
select e.ename as "이름", d.dname as "부서이름" from emp e, dept d where e.deptno = d.deptno
And e.ename like '%A%';
...
select e.ename as "직원이름", d.dname as "부서명", to_char(e.sal) || '원' as "월급" from emp e, dept d 
...
select e.job as "업무", e.ename as "직원이름", d.dname as "부서이름" from emp e, dept d 
    where e.deptno = d.deptno
And e.job ='SALESMAN';
...
select e.empno as "직원번호",  e.ename as "직원이름", e.sal *12 as "연봉", e.sal*12+e.comm as "실급여", s.grade as "급여등급"  from emp e, salgrade s 
...
select e.deptno as "부서번호", d.dname as "부서이름", e.ename as "직원이름", e.sal as "월급", s.grade as "급여등급" from emp e, dept d,  salgrade s 
    where e.deptno = d.deptno
    And e.deptno = 10
    and e.sal between s.losal and s.hisal;
...
select nvl(e.ename, '미정') as "이름", e.deptno as "부서번호", d.dname as "부서이름" from emp e, dept d
    where e.deptno(+) = d.deptno;
...
select e.ename as "이름", nvl(e.deptno,'0') as "부서번호", d.dname as "부서이름" from emp e, dept d
    where e.deptno = d.deptno(+);
...
select e.ename as "이름" , e.sal as "월급", s.grade from emp e, dept d, salgrade s
    where e.deptno = d.deptno
    and d.loc = 'DALLAS'
    and e.sal between s.losal and s.hisal;
...
select nvl(e.ename, '누구?') as "이름", nvl(to_char(d.deptno),'어디?') as "부서번호", nvl(to_char(d.dname),'어디?') as "부서이름" from emp e, dept d
    where e.deptno(+) = d.deptno
union
select nvl(e.ename, '누구?') as "이름", nvl(to_char(d.deptno),'어디?') as "부서번호", nvl(to_char(d.dname),'어디?') as "부서이름" from emp e, dept d
    where e.deptno = d.deptno(+)
...
select e.empno as "사원번호", e.ename as "사원이름", nvl(to_char(m.empno),'없음') as "관리자번호", nvl(to_char(m.ename),'없음') as "관리자이름" from emp e, emp m
    where e.mgr = m.empno(+);
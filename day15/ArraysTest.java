...
import java.util.Arrays;	// 배열과 관련된 유용한 기능을 가지고 있는 것이 Arrays
...
public class ArraysTest {
	...
	public static void main(String[] args) {
		...
		Arrays.sort(ary);	//sort는 정렬 알고리즘이다. (순서대로 나열한다.)
		...
		for (int i = 0; i < ary.length; i++)
			objAry[i] = ary[i];
		List<Integer> integerList = Arrays.asList(objAry); //	배열을 list로 만들 수도 있다.

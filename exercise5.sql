...
select e.ename as "이름", e.job as "업무", d.dname as "부서이름" from emp e join dept d using (deptno)
where d.loc = 'DALLAS';
...
select e.ename as "이름", d.dname as "부서이름" from emp e join dept d using (deptno)
...
select e.ename as "직원이름", d.dname as "부서명", to_char(e.sal,'9,999') || '원' as "월급" from emp e join dept d 
   using (deptno)
where e.sal >=3000;
...
select e.job as "업무", e.ename as "직원이름", d.dname as "부서이름" from emp e join dept d 
    using (deptno)
...
select e.empno as "직원번호",  e.ename as "직원이름", e.sal *12 as "연봉", e.sal*12+e.comm as "실급여", s.grade as "급여등급"  from emp e join salgrade s 
...
select e.deptno as "부서번호", d.dname as "부서이름", e.ename as "직원이름", e.sal as "월급", s.grade as "급여등급" from emp e join dept d
...
select nvl(e.ename, '미정') as "이름", e.deptno as "부서번호", d.dname as "부서이름" from emp e right join dept d
    on e.deptno = d.deptno;
...
select e.ename as "이름", nvl(e.deptno,'0') as "부서번호", d.dname as "부서이름" from emp e left join dept d
    on e.deptno = d.deptno;
...
select e.ename as "이름" , e.sal as "월급", s.grade from emp e join dept d
    on e.deptno = d.deptno
join salgrade s on e.sal between s.losal and s.hisal
     where d.loc = 'DALLAS';
...
select nvl(e.ename, '누구?') as "이름", nvl(to_char(d.deptno),'어디?') as "부서번호", nvl(to_char(d.dname),'어디?') as "부서이름" from emp e full join dept d
    on e.deptno = d.deptno;
...
select e.empno as "사원번호", e.ename as "사원이름", nvl(to_char(m.empno),'없음') as "관리자번호", nvl(to_char(m.ename),'없음') as "관리자이름"
